...
public class ScrollerView extends ViewGroup {
	...
	private OnScrollFinish finisher;
	...
	private final Paint selectedTabPaint = new Paint();
	private Canvas canvas;

	private Bitmap screenIndicator;

	// private OnLoadListener load;

	private RectF bar;

	private final Paint tabIndicatorBackgroundPaint = new Paint();
	...
	private void initWorkspace() {
		...
		selectedTabPaint.setColor(0xaa999999);
		selectedTabPaint.setStyle(Paint.Style.FILL_AND_STROKE);

		tabIndicatorBackgroundPaint.setColor(0xaa777777);
	...
	public void computeScroll() {
		...
		if (scroller.isFinished() && finisher != null)
			finisher.onFinish();
	}

	public void setOnScrollFinish(OnScrollFinish arg1) {
		finisher = arg1;
	}

	public OnScrollFinish getOnOnScrollFinish() {
		return finisher;
	}
	 ...
	 * The following implementation attempts to fast-track the drawing dispatch by
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (fastDraw) {
			...
			} else { // If we are scrolling, draw all of our children
				final int count = getChildCount();
				for (int i = 0; i < count; i++)
		...
		updateTabIndicator();
		canvas.drawBitmap(screenIndicator, getScrollX(), 3, paint);
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			// We can still get here even if we returned false from the intercept
			// function.
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (touchState == TOUCH_STATE_SCROLLING) {
				...
				// 2.1 bug.
				// We need to do the work of interceptTouchEvent here because we don't
				// intercept the move
