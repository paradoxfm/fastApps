...
import ru.megazlo.fastfilehd.components.RowData;
...
public class Sets {
	...
	public static void restoreLists(fmMain act) {
		LinearLayout lay = (LinearLayout) act.findViewById(R.id.LayMain);
		LinearLayout.LayoutParams lpar = new LinearLayout.LayoutParams(act.getWindowManager().getDefaultDisplay()
				.getWidth() / 2, act.getWindowManager().getDefaultDisplay().getHeight());
		ListBase lst = null;
		if (Sets.dat.size() == 0) {
			lst = new ListSDC(act, new RowDataSD(), false);
			lay.addView(lst, lpar);
			lst = new ListSDC(act, new RowDataSD(), false);
			lay.addView(lst, lpar);
		} else {
			for (RowData dt : dat) {
				if (dt.getClass() == RowDataSD.class)
					lst = new ListSDC(act, (RowDataSD) dt, true);
				if (dt.getClass() == RowDataFTP.class)
					lst = new ListFTP(act, (RowDataFTP) dt, true);
				lay.addView(lst, lpar);
			}
		}
	}
