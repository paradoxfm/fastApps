...
import ru.megazlo.fastfile.components.list.ListBase;
import ru.megazlo.fastfile.components.list.ListFTP;
import ru.megazlo.fastfile.components.list.ListSDC;
...
public class MenuChecker {
	...
	public static void remList(ListBase lst) {
		if (fmMain.CONTEXT.scrv.getChildCount() == 1)
			exitApp(fmMain.CONTEXT);
		if (lst.getClass() == ListSDC.class)
			Sets.dat.remove(((ListSDC) lst).getDat());
		if (lst.getClass() == ListFTP.class)
			Sets.dat.remove(((ListFTP) lst).getDat());
		removeList(fmMain.CONTEXT, lst);
	}

	private static void removeList(fmMain act, ListBase lst) {
